...
# src = np.float32([[596, 439], [679, 438], [251, 667], [1052, 670]])
# dst = np.float32([[0-133, 0-7.5], [IMAGE_W+20, 5], [0, IMAGE_H], [IMAGE_W, IMAGE_H]])
...
binary_warped = cv2.warpPerspective(result, M, (IMAGE_W, IMAGE_H)) # Image warping
...
def fit_polynomial(binary_warped):
    ...
    leftx, lefty, rightx, righty, out_img = get_fits_by_sliding_windows(binary_warped, line_lt, line_rt, n_windows=9, verbose=False)
...
def get_fits_by_previous_fits(binary_warped, line_lt, line_rt, verbose=False):
    """
    Get polynomial coefficients for lane-lines detected in an binary image.
    This function starts from previously detected lane-lines to speed-up the search of lane-lines in the current frame.
    :param binary_warped: input bird's eye view binary image
    :param line_lt: left lane-line previously detected
    :param line_rt: left lane-line previously detected
    :param verbose: if True, display intermediate output
    :return: updated lane lines and output image
...
# final_out = draw_back_onto_the_road(image, Minv, line_lt, line_rt, True)
...
if __name__ == '__main__':
    ...
    line_lt, line_rt, img_out = get_fits_by_sliding_windows(binary_warped, line_lt, line_rt, n_windows=7, verbose=True)

    
    final_out = draw_back_onto_the_road(image, Minv, line_lt, line_rt, True)
    cv2.imshow('output_final',final_out)
    cv2.waitKey(0)
    plt.show()
